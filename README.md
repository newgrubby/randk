...
(Орехово-Зуево · Павловский Посад · Электросталь — 4 офиса в 3 городах).
...
Все кнопки открывают контактное окно: выбор города → выбор офиса → звонок,
...
| Ссылки VK и MAX, реквизиты                   | `src/content/site.ts`                 |
...
и нужным `citySlug`. Город автоматически покажет его отдельным блоком — так
устроены два офиса Павловского Посада.
...
Объект в `src/content/teachers.ts`. Без `name` карточка отображается нейтрально
(только специализация) — так сделано намеренно, пока нет реальных данных.
...
- [ ] подтверждены адреса, телефоны и графики четырёх офисов
