...
**Node.js 24.x** — версия зафиксирована в трёх местах, которые должны совпадать:
`engines.node` в `package.json`, `.nvmrc` и базовый образ в `Dockerfile`.

Мажорная версия закреплена намеренно: открытый диапазон вида `>=20.9.0` означает, что
хостинг молча переедет на новый мажор Node при его выходе, и сборка может сломаться
