 ...
 * Физический офис.
 *
 * Отдельная сущность, а не синоним города. Каждый подтверждённый офис даёт
 * собственную запись LocalBusiness.
 */
export type Office = {
  id: string;
  citySlug: CitySlug;
  ...
  phone: string | null;
  /** Ссылка на подтверждённый контакт офиса в MAX. null — кнопку не показываем. */
  maxUrl: string | null;
  /** Строки графика. Пустой массив — график не подтверждён, не выводится. */
  schedule: string[];
  /** Deeplink построения маршрута в Яндекс Картах. null — кнопку не показываем. */
  yandexRouteUrl: string | null;
  /** Точная широта и долгота офиса. null — карта и координаты в разметке не выводятся. */
